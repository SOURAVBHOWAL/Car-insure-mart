...
app.post('/send-otp', async (req, res) => {
    var  panID,dob = req.body;
    console.log(req.body);
    let panholder;
    const panholders = await Pan.find();
    panholders.forEach(ele => {
        if(ele.panId===req.body.panId && ele.dob===req.body.dob){
            panholder=ele;
        }
    });
    console.log(JSON.stringify(panholder));
    if (!panholder) {
        return res.status(404).json({ message: 'PAN ID not found//DOB not matching' });
    }
    try{
        console.log(JSON.stringify(panholder));
...
app.post('/upload2', upload2.single('file'), async (req, res) => {
    try {
        ...
        if(flaskResponse.data.gate1_result){
            ...
            if (updatedRegister && flaskResponse.data.gate2_result) {
